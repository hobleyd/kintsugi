...
#
# REPAIR NOTE (2026-09-10, after failure on HTWHPBCD6106928):
...
# installer reported success while silently doing nothing. `-suppressUpToDateInfo` is not
# a documented DisplayLink switch; the name itself indicates it only suppresses the
# informational dialog the installer shows when ITS OWN internal check decides the
# machine is already sufficiently up to date - it does not force a real install, and that
# internal check is not guaranteed to agree with the "Software Package Version" string we
# scrape from the release notes page. That mismatch is the most likely explanation for a
# clean exit code with no actual change. This script's general contract also requires
# passing whatever silent/no-restart switches the installer needs, which the prior
# version failed to do (no `-norestart` was passed at all). Fixed by dropping the
# unverified flag and using only the documented `-silent -norestart` pair; since we
# already gate the installer invocation behind our own "installed < latest" comparison,
# we don't need the installer's own up-to-date guard/suppression, and the final registry
# re-read after install still catches any case where the installer silently no-ops again.
...
try {
    ...
    try {
        ...
        $installProcess = Start-Process -FilePath $installerPath -ArgumentList @('-silent', '-norestart') -Wait -PassThru
