...
# REPAIR NOTE (2026-09-11): Previous run failed with "The remote server returned an
# error: (404) Not Found" while downloading the Windows installer for the version it
# had just determined was latest (3.15.0). The only network call in the script that
# builds a URL directly from the discovered version string is the installer download,
# and the version was discovered by scraping the directory index at
# https://www.python.org/ftp/python/ and picking the highest "X.Y.Z/" folder name.
# That index is not a reliable "fully published and ready" signal: CPython's release
# infra is known to create/expose a version's folder (and can publish the source
# tarball) before every platform's binaries - including the Windows amd64 installer -
# have finished uploading, and today's date (2026-09-11) is before CPython's usual
# October final-release date for a new X.Y.0, which is consistent with 3.15.0's
# directory having appeared without a complete Windows artifact set behind it yet.
# Fix: replaced the FTP directory scrape with the redirect-based GitHub check this
# tool's own guidance recommends for GitHub-hosted projects. CPython's source and its
# tagged releases are hosted at github.com/python/cpython, and GitHub only points
# releases/latest at a release the maintainers have explicitly published and NOT
# flagged as a pre-release - so an in-progress/partial cut can no longer be mistaken
# for "latest". Everything else (registry checks, process shutdown, MSI/EXE install,
# post-install verification) is unchanged from the previous version because none of
# it was implicated in this failure.
#
...
#
# WARNING: The new GitHub-redirect version check assumes python/cpython keeps
# publishing a proper (non-draft, non-prerelease) GitHub Release for every final
# X.Y.Z tag, as it has historically done. It also assumes that by the time such a
# release is marked "latest", python.org's Windows binaries for that version are
# already in place - which is far more likely than with a raw FTP directory
# listing, but is not a contractually guaranteed atomic operation. This script has
# no live web access at authoring time to re-confirm current behavior; if this
# assumption ever breaks, the failure mode is the same 404 this repair fixes, and
# will surface the same way (non-zero exit, clear stderr message) rather than a
# silent bad install.
...
function Get-LatestPythonVersion {
    $releaseUri = 'https://github.com/python/cpython/releases/latest'
    $response = $null
    try {
        $response = Invoke-WebRequest -Uri $releaseUri -MaximumRedirection 0 -SkipHttpErrorCheck
    }
    catch {
        throw "Failed to query latest CPython release from $releaseUri : $($_.Exception.Message)"
    }
    ...
    if ([string]::IsNullOrEmpty($location)) {
        throw "No redirect Location header returned from $releaseUri (HTTP status $($response.StatusCode))."
    }

    $tag = ($location.TrimEnd('/') -split '/')[-1]
    $latest = $tag.TrimStart('v', 'V')
    if ($latest -notmatch '^\d+\.\d+\.\d+$') {
        throw "Unexpected release tag format from $releaseUri : '$tag'"
    }
