...
#
# REPAIR NOTE (2026-09-10, after 1st failure on HTWHPBCD6106928):
# The prior version of this script invoked the installer with
# `-silent -suppressUpToDateInfo` and no `-norestart` at all. That run exited 0 and the
# install appeared to "succeed", but the post-install registry re-read still showed the
# pre-upgrade version (12.2.2412.0) instead of the target (12.2.2440.0) - i.e. the
# installer reported success while silently doing nothing. Fixed (at the time) by
# dropping that flag and switching to `-silent -norestart`.
#
# REPAIR NOTE (2026-09-11, after 2nd failure on HTWHPBCD6106928):
# The 2026-09-10 fix itself was wrong: `-norestart` is not a switch this EXE installer
# understands at all - it's the msiexec/MSI-deployment convention. DisplayLink's own
# knowledge base ("Installation switches for installing DisplayLink software on
# Windows") documents the setup.exe switches as -silent, -suppressUpToDateInfo,
# -stageDrivers, -uninstall, -noreboot, and -ignoreCompatibility - there is no
# "-norestart" among them. Passing an argument the installer doesn't recognize is the
# most likely explanation for the clean, immediate exit code 1 seen on this run (as
# opposed to the silent no-op with exit 0 seen on the previous run). Note also that the
# correctly-named reboot-suppression switch, -noreboot, is documented as "not required
# on Windows 10/11" and "removed since R11.4 software" - since 12.2.2440.0 is far newer
# than R11.4, passing -noreboot risks the exact same unrecognized-argument failure.
# Fixed by dropping the restart-control switch entirely and passing only -silent, which
# is documented as valid across installer versions; no explicit reboot suppression is
# needed since DisplayLink does not require a reboot on Windows 10/11 for this install
# path, and the final registry re-read below still catches any case where the install
