...
impl Presentation {
    ...
    /// How much notice this cycle gives before it starts. Zero for an unattended run, which has
    /// nobody to warn — see `run_unattended`.
    fn warning(self) -> Duration {
        match self {
            Presentation::Interactive { warning } => warning,
            Presentation::Unattended => Duration::ZERO,
        }
    }
