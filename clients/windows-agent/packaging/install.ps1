...
# Keep this file pure ASCII. Windows PowerShell 5.1 decodes a BOM-less .ps1 as cp1252 rather than
# UTF-8, so a UTF-8 em dash arrives as three characters ending in U+201D - which PowerShell honours
# as a double-quote delimiter. One em dash inside a string literal closes that string early and the
# parse collapses into "Missing closing '}'" errors pointing at lines nowhere near the real one.
# That shipped in v0.11.0 and made the installer unparseable on every host it reached, including
# under the bootstrap script rendered by WindowsBootstrapScript.cs. A BOM would also fix it, but a
# BOM is invisible and any editor that normalises it away silently re-arms the bug; ASCII is the
# rule, and tests/packaging_scripts.rs is what enforces it.
