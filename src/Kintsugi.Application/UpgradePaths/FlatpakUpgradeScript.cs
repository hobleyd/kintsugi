...
/// <summary>
/// Builds the fixed --appName/--appId/--update-version/--update script for everything Flatpak
/// manages, Flatpak itself included. Shared between the "Find Upgrade Paths" research flow
...
/// — see <c>IUpgradePathRepository.FindExistingSignatureForScriptAsync</c>.
/// </para>
/// <para>
/// One text for Flatpak's own row and every application it manages (see
/// <see cref="RecognizedPackageManager.BuildScript"/>): Flatpak's own row is told
/// apart at runtime by <c>--appName</c> — the name the Linux agent reports it under
/// (<c>system_info::FLATPAK_NAME</c>) — because the Applications screen shows a manager's script
/// once, on the manager's row, on behalf of every application nested under it, and that is only
/// honest if the manager's bytes are its children's bytes. See the remarks on
/// <see cref="HomebrewUpgradeScript"/>, where the reasoning lives in full. Flatpak is not a Flatpak
/// — it is a distribution package — so that row has no version to answer and a different upgrade,
/// both described inside the script; hence the branch rather than Snap's plain reuse.
/// </para>
/// <para>
...
public static class FlatpakUpgradeScript
{
    public static string Build()
    {
        return """
            ...
            # Flatpak's own row. The agent reports Flatpak itself as an application named "Flatpak"
            # (see system_info::FLATPAK_NAME in the Linux agent), and it is not a Flatpak: Flathub does
            # not carry it and `flatpak update flatpak` is meaningless. One script serves it and
            # everything it manages so that one review covers all of them; this is where the two part
            # ways.
            is_flatpak_itself() {
              [ "$(printf '%s' "$APP_NAME" | tr '[:upper:]' '[:lower:]')" = "flatpak" ]
            }

            latest_version() {
              local response version

              if is_flatpak_itself; then
                # Flatpak is a distribution package, and that makes its own row the one case in this
                # catalog with no answerable version question.
                #
                # The obvious implementation (read the latest tag from github.com/flatpak/flatpak) is
                # actively harmful, and was written and then removed rather than shipped. It reports
                # what upstream released; --update can only install what this host's repositories
                # carry, and those are years apart — Debian 12 ships 1.14.10 against an upstream
                # 1.18.2. The row would say "update available" forever, --update would exit 0 having
                # changed nothing, the agent would report the upstream version as installed, and the
                # next inventory would contradict it. A patch that always succeeds and never changes
                # anything is worse than no patch at all, because nothing looks wrong.
                #
                # So this declines to answer. LatestVersion stays null, which means updateAvailable is
                # false, which means the agent never tries — and flatpak still gets patched, by the
                # same `apt-get upgrade`/`dnf upgrade` that patches every other distribution package
                # (see the Linux agent's `os_update`). This is the same reasoning that keeps apt and
                # dnf themselves out of PackageManagerCatalog, applied to the one member that turns out
                # to be one of their packages.
